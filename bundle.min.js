...
( function( PCG ){
    ...
    const domEl = function( type, cfg = {} ){
        const cls = cfg.cls, style = cfg.style, attr = cfg.attr, prop = cfg.prop, on = cfg.on, renderTo = cfg.renderTo,
            el = cfg.el || document.createElement( type ), classList = el.classList;
        let i, _i;
        if( cls ){
        ...
        for( i = 2, _i = arguments.length; i < _i; i++ ){
            let child = arguments[ i ], type = typeof child;
            if( type !== "object" ){
                child = D.Text( child )
            }
    ...
    const D = PCG.D = { svg: null, label: null, div: null, path: null, Text: val => document.createTextNode( val ) };
...
( function( PCG ){
    ...
    PCG.updateYAxis = function updateYAxis( minMax, getY ){
        ...
        const delta = Math.ceil( minMax.max ) - Math.floor( minMax.min ), from = minMax.min;
        ...
        const basic = delta / ( count - 1 ),
            ...
            rounded = Math.round( basic / scale ) * scale, roundedFrom = Math.ceil( from / scale * 10 ) * scale / 10;
        ...
        for( i = 0; i < count; i++ ){
            ...
            if( pos < graphHeight * 1.01 && pos > .05 ){
                val = out[ i ];
                usedHash[ val ] = true;
                if( val in hash ){
                    ...
                    hash[ val ] = {
                        ...
                        visible: false,
                        destroy: false,
                        line: D.div( { cls: "pcg-y-axis", renderTo: yAxisStorage, style: { top: pos + "px" } } ),
                        label: D.div( {
    ...
    PCG.updateXAxis = function updateYAxis(){
        ...
        const from = this.frame.from, to = this.frame.to, delta = to - from, granule = delta / 6,
            bigBang = this.data[ 0 ][ 0 ], initialTimeOffset = bigBang % granule,
        ...
        for( i = 0; i < 7; i++ ){
            ...
            this.els.xAxisLabelsStorage.appendChild( D.div( {
...
( function( PCG ){
    ...
    PCG.initCheckboxes = function initCheckboxes(){
        const switchesEl = this.renderTo.switches, updateVisible = () => {
            this._visible = list.filter( el => el.show ).map( el => el.i );
            this.update();
            this.updateNav()
        };
        ...
        this.columns.forEach( ( name, i ) => {
            ...
            list.push( dataRow );
            D.label( { cls: "pcg-checkbox-wrapper", renderTo: switchesEl }, D.input( {
...
( function( PCG ){
    ...
    PCG.initDOM = function initDOM(){
        ...
        this.els = {
            ...
            navEars: [ D.div( {
                ...
                cls: "navEar",
                style: { left: 0 }
            } ), D.div( { renderTo: around.nav, cls: "navEar", style: { right: 0 } } ) ],
    ...
    PCG.initListeners = function initListeners(){
        ...
        let world;
        let frame;
        let frameWidth, startFrame, pxToTime;
        let toLeft = true;
        const move = e => {
        ...
        const down = e => {
            ...
            this.els.navExpandControl.addEventListener( "touchmove", touchMove );
        ...
        this.els.navMoveControl.addEventListener( "mousedown", e => {
            ...
            const frameWidth = frame.to - frame.from, startFrame = frame.from;
            const move = e => {
                let point = [ e.clientX, e.clientY ];
                frame.from = startFrame + ( point[ 0 ] - start[ 0 ] ) / world.nav.width * ( this.maxDate - this.minDate );
                if( frame.from < this.minDate ){
                    frame.from = this.minDate
                }
        ...
        this.renderTo.graph.addEventListener( "mousemove", function( e ){
            const x = e.offsetX;
            const time = _self.xToTime( x ), sliceID = _self._binarySearch( time ), getX = _self.getX,
                data = _self.data;
...
( function( PCG ){
    ...
    PCG.numberFormat = function( num ){
        const strNum = num + "";
        let i, out = [], _i = strNum.length, count = _i % 3 || 3, last = count;
...
( function( PCG ){
    ...
    PCG.updateGraph = function updateGraph(){
        const limits = { from: this._binarySearch( this.frame.from ) - 1, to: this._binarySearch( this.frame.to ) + 1 };
        let minMax = this._getMinMax( limits.from, limits.to );
        if( this.camera === null ){
            this.camera = { minMax: minMax }
        }else{
            if( Math.abs( this.camera.minMax.max - minMax.max ) > .01 ){
                this.update()
            }
        ...
        const maxDotsCount = this.world.graph.width / this.consts.graphPxPerDot, minDate = this.frame.from,
            momentumDelta = this.frame.to - minDate, momentumInGranule = momentumDelta / maxDotsCount, data = this.data,
            visible = this._getVisible(), width = this.world.graph.width, height = this.world.graph.height,
            ...
            svgData = visible.map( ( id, i ) => [ [ getX( data[ limits.from ][ 0 ] ), getY( data[ limits.from ][ visible[ i ] + 1 ] ) ] ] );
        this.updateYAxis( minMax, getY );
        let lastDate = data[ limits.from ][ 0 ], lastSlice = data[ limits.from ], graph, i, _i, j, _j, k, _k, slice;
        ...
        for( i = limits.from + 1, _i = limits.to; i <= _i; i++ ){
            ...
            if( slice[ 0 ] - lastDate >= momentumInGranule ){
                _j = oddList.length;
                let tDiff = oddList[ _j - 1 ][ 0 ] - oddList[ 0 ][ 0 ] - .01,
                    centerTime = ( oddList[ _j - 1 ][ 0 ] + oddList[ 0 ][ 0 ] ) / 2, startTime = oddList[ 0 ][ 0 ],
                    midValue, dataRow, dt, dv, dt2dv2, maxDt2dv2, maxSlice;
...
( function( PCG ){
    ...
    PCG.updateNav = function updateNav(){
        ...
        const svgData = this._all.map( ( id, i ) => [ [ 0, getY( data[ 0 ][ i + 1 ] ) ] ] );
        let i = 1, _i = data.length, sliceSum = this._all.map( () => 0 ), sliceSumCount = 0;
        const next = () => {
            if( updateNavID !== this.updateNavID ) return;
            for( let _i0 = Math.min( _i, i + 1e4 ); i < _i0; i++ ){
                const slice = data[ i ];
...
( function( PCG ){
    ...
    PCG.prototype = {
        ...
        formatters: { date: PCG.dateFormatter, weekDate: PCG.weekDateFormatter },
        els: null,
        minDate: null,
        maxDate: null,
        world: null,
        ...
        _getMinMax: function( from, to ){
            const data = this.data;
            let min = Infinity, max = -Infinity, i, j, slice, point;
            const visible = this._getVisible();
        ...
        showTooltip: function( sliceID ){
            const slice = this.data[ sliceID ], time = slice[ 0 ];
            const visible = this._getVisible();
            ...
            for( i = 0, _i = visible.length; i < _i; i++ ){
                ...
                this.els.tooltipInfo.appendChild( D.div( {
                    cls: "pcg-tooltip__info-item",
                    style: { color: this.colors[ name ] }
                }, D.div( { cls: "pcg-tooltip__info-item__count" }, PCG.numberFormat( val ) ), this.names[ name ] ) )
            }
        ...
        collectWorldInfo: function(){
            ...
            this.world = {
                nav: { width: navRect.width, height: navRect.height },
                graph: { width: graphRect.width, height: graphRect.height - this.consts.XAxisHeight }
            }
        },
        resize: function(){
            requestAnimationFrame( () => {
