(function(PCG){
    ...
    const magnitude4 = function(x1,y1,x2,y2) {
        let tmp;
        return sqrt((tmp=(x1-x2))*tmp+(tmp=(y1-y2))*tmp)
    };
    const magnitude2 = function(p1,p2) {
        return magnitude4(p1[0],p1[1],p2[0],p2[1])
    };
    const normalize2 = function() {
    ...
    PCG.GL = function(canvas, cfg) {
        this.c = canvas;
        this.gl = this.c.getContext('experimental-webgl', Object.assign({
            depth: false,
            alpha: true,
    ...
    PCG.GL.prototype = {
        ...
        init: function() {
            ...
            // Attach a vertex shader // Attach a fragment shader
            gl.attachShader( shaderProgram, vertShader );
            gl.attachShader( shaderProgram, fragShader );

            // Link both the programs
            gl.linkProgram( shaderProgram );
            this.programm = shaderProgram;
        },
        clear: function() {
            this.scene.length = 0;
        },
